...
На связи Татьяна Шустилова, руководитель WonderLang School.

Пожалуйста, ответитьте на пару вопросов, чтобы я подобрала для вас подходящий тест👇`,
				{
					keyboard: builder([{ label: "К вопросам" }]).oneTime(true),
				},
			);
			break;
		case "К вопросам":
...
Приступим?👇`,
				{ keyboard: builder([{ label: "Пройти тест" }]).oneTime(true) },
			);
			break;
		case "Нет":
			await updateDeal(userInfo.amoId, undefined, [
...
Если пока нет, то вот что нужно сделать👇`,
				{ keyboard: builder([{ label: "Что нужно сделать?" }]).oneTime(true) },
			);
			break;
...
Вы хотите узнать, как закрыть пробелы всего за несколько недель?`,
				{ keyboard: builder([{ label: "Как починить базу?" }]).oneTime(true) },
			);
			break;
...
По любым вопросам пишите сюда: https://vk.com/im?sel=-213318868`,
			...
			await ctx.send(
				...
				{ keyboard: builder([{ label: "Хочу попробовать" }]).oneTime(true) },
			);
			break;
...
Хотите?`,
				{ keyboard: builder([{ label: "Открыть тайну" }]).oneTime(true) },
			);
			break;
		case "Открыть тайну":
			await ctx.send(
...
Рассказываю👇`,
				{ keyboard: builder([{ label: "Что делать?" }]).oneTime(true) },
			);
			break;
		case "Что делать?":
			await ctx.send(
...
Рассказать, что еще необходимо сделать, чтобы заговорить на английском грамотно и свободно?👇`,
				{ keyboard: builder([{ label: "Что еще сделать?" }]).oneTime(true) },
			);
			break;
...
`,
				{ keyboard: builder([{ label: "Как сделать прорыв?" }]).oneTime(true) },
			);
			break;
...
Тогда вот с чего предлагаю начать👇`,
					{ keyboard: builder([{ label: "С чего начать?" }]).oneTime(true) },
				);
			} else {
				await ctx.send(
			...
			Рассказываю👇`,
					{ keyboard: builder([{ label: "А конкретнее?" }]).oneTime(true) },
				);
			}
			break;
		case "А конкретнее?":
...
Получите ответы на эти вопросы👇`,
				{ keyboard: builder([{ label: "Получить ответы" }]).oneTime(true) },
			);
			break;
